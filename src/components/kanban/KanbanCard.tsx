  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-1">
        ...
        <span className="text-xs text-gray-400 flex-shrink-0 ml-2">ID: #{lead.id.slice(-5)}</span>
      </div>
      ...
      <div className="flex justify-between items-center text-xs mb-2">
        <span 
            ...
            aria-label={`Lead value: ${lead.value}`}
        >
            ${lead.value.toLocaleString()}
        </span>
        {lead.assignedTo && (
            <div className="flex items-center text-gray-500" title={`Assigned to ${lead.assignedTo}`}>
                 {lead.avatarUrl ? (
